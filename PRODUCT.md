...
当前原型包含首页、跑步页、跑后记录页和跑步记录页。点击 GO 后开始真实前端计时，支持暂停和继续，并将最终时长传给跑后记录页；跑后记录可选择天气、心情和见闻；首页可直接进入第四页，以当前会话最近一次保存的记录展示 Year / Month / Day 视图，无记录时显示空状态。不接数据库、网络、定位或持久化；会话内新保存会替换上一条记录。
