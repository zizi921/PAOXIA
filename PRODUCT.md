...
当前原型包含首页、跑步页、跑后记录页和跑步记录页。点击 GO 后开始真实前端计时，支持暂停和继续，并将最终时长传给跑后记录页；跑后记录可选择天气、心情和见闻；首页可直接进入第四页，以本机 Storage 中已完成的多条记录展示 Year / Month / Day 视图和真实统计，无记录时显示空状态。同一天的多次记录可从 Month 分别进入详情，关闭重开后仍保留。不接数据库、网络或定位；未完成跑步和表单草稿暂不恢复。
