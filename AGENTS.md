...
- 当前交付为微信原生小程序，已完成跑步记录使用本机 Storage 持久化并支持多条记录；未完成跑步也保存在本机，可恢复时长和暂停状态，点击 Done 后计时信息转入跑后草稿，支持继续同一次跑步，填写期间不计时；距离在跑后表单手动填写，不申请定位权限；保存或放弃后清除。不连接 CloudBase、数据库或网络。跑后表单仅保留一份本机草稿，可从首页继续填写，保存成功或主动放弃后清除。
