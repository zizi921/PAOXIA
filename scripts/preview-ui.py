...
for name in ['home', 'run', 'recap']:
    ...
    markup = markup.replace('{{paused ? copy.breath : copy.moving}}', 'Moving').replace('{{paused ? copy.resume : copy.pause}}', 'Pause')
    markup = markup.replace('{{hasActiveRun ? copy.continueRun : (hasDraft ? copy.continueDraft : copy.go)}}', 'GO')
    markup = markup.replace('{{editing ? copy.edit : copy.so}}', 'So?').replace('{{editing ? copy.saveChanges : copy.save}}', 'Save →')
    preview_copy = {
        'copy.history': 'Days Out.', 'copy.weather': 'Weather', 'copy.howWasIt': 'How was it?',
        'copy.good': 'Good', 'copy.calm': 'Calm', 'copy.tired': 'Tired', 'copy.unsure': 'Not sure',
    ...
    js = "document.querySelector('[bindtap=go]')?.addEventListener('click',()=>location.href='run.html');let paused=false,seconds=0;const timer=document.querySelector('.timer');if(timer)setInterval(()=>{if(!paused){seconds++;const h=String(Math.floor(seconds/3600)).padStart(2,'0'),m=String(Math.floor(seconds%3600/60)).padStart(2,'0'),s=String(seconds%60).padStart(2,'0');timer.textContent=`${h}:${m}:${s}`}},1000);document.querySelector('[bindtap=togglePause]')?.addEventListener('click',e=>{paused=!paused;e.target.textContent=paused?'Resume':'Pause';document.querySelector('.headline').textContent=paused?'Take a breath.':'Moving'});document.querySelector('[bindtap=finish]')?.addEventListener('click',()=>location.href='recap.html');document.querySelector('[bindtap=save]')?.addEventListener('click',()=>location.href='home.html');document.querySelectorAll('.choice').forEach(el=>el.addEventListener('click',()=>{el.parentElement.querySelectorAll('.choice').forEach(x=>x.classList.remove('selected'));el.classList.add('selected')}));"
