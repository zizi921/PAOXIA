...
for name in ['home', 'run', 'recap']:
    ...
    css = (source / 'styles/handwriting.wxss').read_text() + (source / 'app.wxss').read_text() + stem.with_suffix('.wxss').read_text()
    css = re.sub(r'@import[^;]+;', '', css).replace('page {', 'body {')
    css = re.sub(r'(-?[\d.]+)rpx', lambda m: f'calc({float(m[1])/7.5} * 1vw)', css)
    js = "document.querySelector('[bindtap=go]')?.addEventListener('click',()=>location.href='run.html');let paused=false;document.querySelector('[bindtap=togglePause]')?.addEventListener('click',e=>{paused=!paused;e.target.textContent=paused?'Resume':'Pause';document.querySelector('.headline').textContent=paused?'Take a breath.':'Out there.'});document.querySelector('[bindtap=finish]')?.addEventListener('click',()=>location.href='recap.html');document.querySelector('[bindtap=save]')?.addEventListener('click',()=>location.href='home.html');document.querySelectorAll('.choice').forEach(el=>el.addEventListener('click',()=>{el.parentElement.querySelectorAll('.choice').forEach(x=>x.classList.remove('selected'));el.classList.add('selected')}));"
